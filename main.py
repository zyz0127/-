...
# 首先使用LoadLines函数处理movie lines.txt
print("\nProcessing corpus...")
lines = loadLines(os.path.join(corpus, "movie_lines.txt"), MOVIE_LINES_FIELDS)
# 接着使用1oadConversations处理上一步的结果，得到conversations
...
class Voc:
    def __init__(self, name):
        ...
        self.word2index = {}
        self.word2count = {}
        self.index2word = {PAD_token: "PAD", SOS_token: "SOS", EOS_token: "E0S"}
        self.num_words = 3  # 目前有S0S，E0S，PAD这3个token。
...
def readVocs(datafile, corpus_name):
    ...
    lines = open(datafile, encoding='utf-8'). \
        read().strip().split('\n')
    # 每行用tab切分成问答两个司子，然后调用normalizeString函数进行处理。
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seq, input_lengths, hidden=None):
        ...
        # 将 input_lengths 转移到 CPU 并转换为 int64
        input_lengths = input_lengths.cpu().to(torch.int64)
        # 调用 pack_padded_sequence
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    # 确保损失不为 None 才进行反向计算和参数更新
    if loss is not None and n_totals > 0:
        # 反向计算
        loss.backward()
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        # 计算 loss 后检查它是否为 None
        if loss is None:
            print("Warning: loss is None, skipping this iteration.")
        else:
            print_loss += loss
